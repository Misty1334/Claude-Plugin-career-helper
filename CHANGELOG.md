...
### Accessibility
- Every new skill carries the standard Accessibility section (`dyslexia_friendly` and `colour_blind` from `career-helper-preferences.md`). In addition, the references and templates say concretely how each new output changes in dyslexia-friendly mode: the market map watchlist, the routines register, and the interview competency map become numbered lists with one labelled line per field instead of wide tables; the weekly update report, market watch update, and interview prep pack are numbered throughout, signposted section by section, refer to files by description, repeat names rather than relying on earlier lines, and present one decision per item; the routines setup flow goes one step at a time with a confirmation between steps. Colour-blind mode needs no changes because every rating and priority is a text label.
