...
- **Scheduling guidance for every surface** in the scheduled routines reference (v1.2): a comparison of Claude Code Desktop local scheduled tasks (recommended; runs on your machine with the folder, needs the app open and the computer awake, one catch-up run on wake), Claude Cowork `/schedule`, system cron or launchd running `claude -p "/career-helper:weekly-update" --permission-mode acceptEdits --permission-prompts none`, and cloud Routines (run without your machine but only see a GitHub repository, so unsuitable for a local folder of CVs), with `/loop` noted as session-scoped and not a scheduler. Each route's setup steps and limits are stated plainly.
- **Weekly market map update routine** (routine 6) among the ready-made prompts, with the same do-not-invent and offer-do-not-apply instructions as the skill.
...
- Getting-started guides updated for fourteen skills: `full-overview.md` (new skill section with examples and the automation paragraph), `skill-tips.md` (tips for the new skill), `getting-the-best-guide.md` (table row and skill count, PDF regenerated), `power-user-strategies.md` (Strategy 9 rewritten around the weekly update), `workflow-planner.md` (new intake option and Pattern I: Employed and Quietly Exploring), `preparation-checklist.md` (new scenario), and the getting-started skill itself (routine list, counts, related skills, v1.14.0)
