...
### Added
- **Market Mapper skill** (`/market-mapper`). Builds an evidenced map of ten to fifteen organisations similar to the user's current or recent employer: a confirmed seed profile (sector, size band, geography, products and customers, ownership), comparables gathered across competitors, adjacent-sector peers, suppliers and customers, function-led postings, regional business press, Companies House, and funding sources, then dated signals per organisation in four groups (hiring, growth, investment, and change, including negative signals), decision makers named only where a public source confirms them (with `[NOT FOUND]` and the search to run where none does), a suggested angle per organisation that traces to a specific signal, and a text-label watch priority (Act now, Warm, Watch, Quiet). A discretion posture is confirmed before the first run: for the employed-and-discreet default, every angle is limited to following, reading, and commenting, and nothing touches LinkedIn settings or messages a hiring manager. The Watchlist and Weekly Update capability re-checks every organisation, reports only what has changed since each organisation's last-checked date, offers (but never applies) removals and additions, flags organisations already in the tracker, and rolls the changes into the map in place. Map to Action routes an Act now organisation to `/career-navigator` (Strategic Networking), `/job-scout` (live roles there), or the tracker. A Watchlist Board View extends the application kanban board mechanism to the map: one card per organisation in four priority columns (Act now, Warm, Watch, Quiet) with column meanings, drag and drop plus keyboard-operable Raise and Lower buttons, an edit dialog, a text `STALE` tag for anything not checked in fourteen days, and export of the Watchlist section as markdown (with the signal group carried through) so hand-made changes flow back into the map, which stays the source of truth. The header is rendered from the data block with `textContent`, so nothing user-supplied is inserted as markup, and a regenerated board detects unexported edits from the previous board and offers to copy them. New references `company-mapping.md`, `market-watch.md`, `market-map-board.md`, `market-map-template.md`, `market-watch-update-template.md`, and `market-map-board-template.html`. Outputs: `market-map.md`, `market-watch/{date}-update.md`, and `market-map-board.html`.
- **Weekly Update command** (`/career-helper:weekly-update`). One unattended pass that runs the tracker standup (overdue first, stalled applications, three things for the week), follow-ups due from application strategies, the market map update (map and board refreshed, offers recorded rather than applied), a check on whether learnings notes are ready to synthesise, and one suggested next skill, then saves a dated report to `updates/{date}-weekly-update.md`. It never asks a question and never invents; anything needing a decision is listed under "Decisions waiting for you". Built so that one prompt can go on any scheduler.
- **Career Routines skill** (`/career-routines`). Scheduling now has its own skill, built around Claude Cowork's scheduled tasks. Set Up a Routine checks which inputs exist, recommends the weekly update first, tailors the prompt, asks the one question that matters (run on your machine with your local folder, which must then be awake, or in the cloud with no local folder), gives the exact `/schedule` steps, and records the routine in a new `routines.md` register that `/career-helper:status` reads. The Routine Library holds seven unattended-safe prompts (the weekly update, then the standup, market monitor, posting reminder, follow-up check, pre-interview nudge, and standalone market map update). Review Your Routines reads the register and output folders, flags stale routines, surfaces open decisions, and works through the likely causes when a task did not run. The guidance distinguishes Desktop Cowork (a local folder; the most feature-rich option for persistence, so every session and scheduled run sees the same files) from cloud Cowork (files live in the conversation's data store, so long-running work means staying in the same conversation, and a scheduled run cannot build on last week's files unless the workspace is on a connected drive). Outside Cowork covers Claude Code Desktop local scheduled tasks, cron or launchd running `claude -p "/career-helper:weekly-update" --permission-mode acceptEdits --permission-prompts none`, and cloud Routines (which only clone a GitHub repository), with `/loop` noted as not a scheduler. New references `cowork-scheduling.md`, `routine-library.md`, and `routines-register-template.md`; the former `getting-started/references/scheduled-routines.md` is retired and its prompts live in the library. Output: `routines.md`.

### Changed
- Plugin and marketplace version bumped to 1.16.0, with descriptions updated for the two new skills (sixteen in total)
- `/career-helper:help` skills table and routing, `/career-helper:quick-start` intake option and routing, and `/career-helper:status` (a weekly update check that surfaces open decisions from the latest report, a market map check that surfaces Act now organisations, offers the weekly update when the map is stale, offers the board view once the map passes eight organisations, and lists the new output files) updated
- Tim career coach agent and skill now list 15 orchestratable skills, with routing judgements for "I'm not actively looking, but I want to keep an ear to the ground" and "can this run by itself each week?"
...
- Getting-started guides updated for fifteen skills: `full-overview.md` (new skill section with examples and the automation paragraph), `skill-tips.md` (tips for the new skill), `getting-the-best-guide.md` (table row and skill count, PDF regenerated), `power-user-strategies.md` (Strategy 9 rewritten around the weekly update), `workflow-planner.md` (new intake option and Pattern I: Employed and Quietly Exploring), `preparation-checklist.md` (new scenario), and the getting-started skill itself (Capability 7 now routes to `/career-routines`, counts, related skills, v1.14.0)
- `/job-scout` and `/career-navigator` related-skills lists point at the market mapper
- **Interview Master preparation rebuilt around a competency map** (skill v1.6.0). Interview Preparation now starts with 8 to 12 competencies derived from the job description, each with why it matters, the evidence in the spec, how likely it is to be tested, the interview style, a text-label CV rating (Strong, Partial, Gap) with the evidence behind it, and what may concern the assessor; a Gap is recorded, never stretched. The top five risks are ranked from the map with a mitigation each. Every likely question carries a specific "what not to say". The concerns section becomes four-step objection handling (acknowledge, reframe, evidence, close) with one entry per risk. Two or three timed openers are given as talking points with a word count, not scripts. A one-page dashboard is written last and placed first: readiness by competency (Ready, Needs work, Gap), top five things they will like, top five concerns, top five stories, top ten questions to practise aloud, and one thing to do today. There is deliberately no overall readiness score and no colour rating, in line with the house style. The map's Gap ratings are read by `/skills-radar` (gap analysis) and its Partial ratings by `/application-optimiser` (CV rewrite, as undersold experience to surface). Feature direction informed by Simon Bucknell's "Interview Prep Machine" prompt (Stand Out Get Hired), reinterpreted with text-label ratings and no invented experience.
- **House style fixes in Interview Master references**: pre-existing US spellings corrected in `interview-prep.md`, `interview-prep-template.md`, and `mock-interview.md` (behavioural, specialising, optimised, organised, emphasise, customisation, organisation, visualise)
- README description, skills table, commands table, features, scheduling tip, workspace tip (Desktop versus cloud Cowork persistence), and output-files table updated

### Accessibility
- Every new skill carries the standard Accessibility section (`dyslexia_friendly` and `colour_blind` from `career-helper-preferences.md`). In addition, the references and templates say concretely how each new output changes in dyslexia-friendly mode: the market map watchlist, the routines register, and the interview competency map become numbered lists with one labelled line per field instead of wide tables; the weekly update report, market watch update, and interview prep pack are numbered throughout, signposted section by section, refer to files by description, and repeat names rather than relying on earlier lines; the routines setup flow goes one step at a time with a confirmation between steps. All of this applies only when the preference is set. One decision per item with a plain "what to do" line in the weekly report is the default for everyone, because it is simply clearer. Colour-blind mode needs no changes because every rating and priority is a text label.
